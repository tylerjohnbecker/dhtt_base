...
class ServerNode (rclpy.node.Node):
	...
	def run_experiment(self):
		...
		topmost_then.node_name = "TopmostOr"
		topmost_then.plugin_name = "dhtt_plugins::OrBehavior"
		topmost_then.type = Node.OR

		self.add_node(topmost_then, 'ROOT_0')

		assigned_name = self.get_node_from_partial_name('TopmostOr')

		self.add_pick_place_to('PickPlace1', 'home', 'depot_1', 10, assigned_name)

		middle_and = Node() 

		middle_and.node_name = 'MiddleAnd'
		middle_and.plugin_name = "dhtt_plugins::AndBehavior"
		middle_and.type = Node.AND

		self.add_node(middle_and, assigned_name)

		assigned_name_or = assigned_name

		assigned_name = self.get_node_from_partial_name('MiddleAnd')

		self.add_pick_place_to('PickPlace2', 'home', 'depot_3', 5, assigned_name)
		self.add_pick_place_to('PickPlace3', 'home', 'depot_5', 5, assigned_name)
		self.add_pick_place_to('PickPlace4', 'home', 'depot_2', 7, assigned_name)

		self.start_tree()

		self.wait_for_node_in_state('moveToDestination1', NodeStatus.WORKING)

		self.interrupt_tree()

		self.wait_for_waiting()

		self.mutate_node_to_type(assigned_name_or, "dhtt_plugins::ThenBehavior")
		self.mutate_node_to_type(assigned_name, "dhtt_plugins::OrBehavior")

		self.start_tree()

		self.wait_for_node_in_state('MiddleAnd', NodeStatus.WORKING)

		middle_and_2 = Node()

		middle_and_2.node_name = "MiddleAnd2"
		middle_and_2.plugin_name = "dhtt_plugins::AndBehavior"
		middle_and_2.type = Node.AND

		self.add_node(middle_and_2, assigned_name_or)

		assigned_name = self.get_node_from_partial_name("MiddleAnd2")

		self.add_pick_place_to('PickPlace7', 'home', 'depot_3', 10, assigned_name)

		bottom_then = Node()

		bottom_then.node_name = "BottomThen"
		bottom_then.plugin_name = "dhtt_plugins::ThenBehavior"
		bottom_then.type = Node.THEN

		self.add_node(bottom_then, assigned_name)

		assigned_name = self.get_node_from_partial_name("BottomThen")

		self.add_pick_place_to('PickPlace5', 'home', 'depot_1', 9, assigned_name)
		self.add_pick_place_to('PickPlace6', 'home', 'depot_5', 15, assigned_name)
