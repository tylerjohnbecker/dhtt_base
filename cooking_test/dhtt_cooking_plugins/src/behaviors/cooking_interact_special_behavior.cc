...
void CookingInteractSpecialBehavior::do_work(dhtt::Node *container)
{
	...
	if (not suc)
	{
		DHTT_LOG_ERROR(this->com_agg, "move_to request did not succeed, returning early: " <<
					 res.get()->error_msg);
		this->done = false;
		return;
	}
	...
	req->action.action_type = CookingBehavior::which_arm(container) == "left_arm"
								  ? dhtt_cooking_msgs::msg::CookingAction::INTERACT_PICK_UP_SPECIAL_ARM1
								  : dhtt_cooking_msgs::msg::CookingAction::INTERACT_PICK_UP_SPECIAL_ARM2;
	...
	if (not suc)
	{
		DHTT_LOG_ERROR(this->com_agg, 
					 "interact_special request did not succeed: " << res.get()->error_msg);
	}
